...
public final class EncodeActivity extends Activity {
    ...
    private static final int EXTERNAL_STORAGE_OK = 200;
        ...
        switch (item.getItemId()) {
            case R.id.menu_share:
                if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1) {
                    if (ContextCompat.checkSelfPermission(EncodeActivity.this,
                            Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                        // 先判断有没有权限 ，没有就在这里进行权限的申请
                        ActivityCompat.requestPermissions(EncodeActivity.this,
                                new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, EXTERNAL_STORAGE_OK);
                    } else {
                        // 说明已经获取到摄像头权限了
                        share();
                    }
    ...
    public void onRequestPermissionsResult(int requestCode
        ...
        if (requestCode == EXTERNAL_STORAGE_OK) {
